...
- A landscape's own building form can keep its own room (`BiomeDef.interiors` `form:ID`, looked up before `house`): `tests/interior/test_interiors.gd` holds that every house opens on its own form's room (the crags' roundhouse, the drowned city's stilt house), and walks each to what is in it.
