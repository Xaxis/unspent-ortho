...
- A landscape's own building form can keep its own room (`BiomeDef.interiors` `form:ID`, looked up before `house`): `tests/interior/test_interiors.gd` holds that every crags roundhouse, and nothing else there, opens on a round room, and walks into every bay between its piers.
