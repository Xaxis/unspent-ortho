...
- 12_landscape: `src/systems/12_landscape.gd`, reached by `tools/tour.sh tours/landscape.tour`; the land materials' raws (GEAR.md §11), each walked up to by name at eye level, by `tools/tour.sh tours/materials.tour` (seed 1) and `tours/materials_caves.tour` (the dripstone, underground); the snowfield's grid hung with ice, on a seed whose grid crosses it, by `tools/tour.sh tours/iced_line.tour` (seed 4).
