...
- Ground above the ground (docs/ABOVE.md, S0-S1): mass hung over tiles (`WorldData.overhead`) is walked under, seen past, jumped and climbed under by its headroom (rules `tools/test.sh test_overhead`) and drawn by `TerrainMesher._spans` (underside, banded rim, lifted top; `tools/test.sh test_spans`). No worldgen makes any yet: stage one with `--above=arch` or `--above=roof` over the start (AboveStage), reached by `tools/tour.sh tours/above-arch.tour --seed=1 --place=moss --above=arch --hour=10.5 --weather=clear:0` and `tools/tour.sh tours/above-roof.tour --seed=1 --place=moss --above=roof --hour=10.5 --weather=clear:0` (from above, then over the shoulder under it). From the top view the mass the player is under is cut at the section plane with an inked cap and its outline chalked on the ground (`43_above`, `AboveMap`, world.gdshader `above_*`; `await above_cut` / `above_whole`; `tools/test.sh test_above_map`).
