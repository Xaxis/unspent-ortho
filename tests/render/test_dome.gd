...
func test_a_sign_under_a_lid_burns_at_noon_and_one_under_an_open_sky_does_not() -> void:
	# `neon_burn` (sky.gdshaderinc) in the numbers, because no test can call a
	# shader function and this is the arithmetic it was given. A NEON-coded face
	# was a function of the sky's mean ALONE, so the Slums' own warm light at noon
	# gave 0.24 — a city whose advertising is GUTTERING, which is the opposite of
	# the one thing that landscape says about itself. The lid is a floor and never
	# a replacement, so an open-sky evening is untouched to the pixel.
	var slums := BiomeRegistry.get_def(&"slums")
	var tint := SkyLight.type_light(slums, NOON)
	var mean := (tint.x + tint.y + tint.z) / 3.0
	var sky_only := clampf((0.9 - mean) * 6.0, 0.0, 1.0)
	lt(sky_only, 0.5, "the sky alone leaves the Slums' signs part-lit at noon (%.2f)" % sky_only)
	gt(maxf(sky_only, slums.sky_shut), 0.9, "and with the lid as a floor they are fully lit")
	eq(BiomeRegistry.get_def(&"coast").sky_shut, 0.0,
		"while the coast's signs are still the sky's business alone")
