...
func test_versions_from_the_future_and_the_distant_past_are_refused_plainly() -> void:
	...
	eq(SaveSlots.problem(1, r.code), "Slot 1 is from a newer game.")
	...
	eq(r.code, &"older")
	eq(SaveSlots.problem(2, r.code), "Slot 2 is too old for this game.")
...
func test_damaged_and_missing_files_never_throw_and_say_so() -> void:
	...
	check(problems.has("The autosave cannot be read."), "said plainly: %s" % str(problems))
	check(problems.has("Slot 1 cannot be read."), "said plainly: %s" % str(problems))
	var o := BootOptions.new()
...
func test_the_title_offers_continue_for_the_newest_and_names_what_cannot_be_read() -> void:
	...
	menu.handle(&"confirm")
	eq(menu.note, "Slot 1 cannot be read.", "and says it again when asked")
	menu.close()
	menu.free()
	Sx.finish()
