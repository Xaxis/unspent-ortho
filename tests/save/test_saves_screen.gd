...
func test_load_from_the_pause_page_replaces_the_game_with_the_saved_one() -> void:
	...
	check(page is UiSavesScreen and (page as UiSavesScreen).mode == &"save", "saves opens over home, saving")
	page.handle(&"confirm")
	eq(page.note, "Saved to slot 1.")
	page.handle(&"back")
	check(ui.call("top") == pause, "esc comes back to home")
	...
	page = ui.call("top")
	page.handle(&"right")
	check(page is UiSavesScreen and (page as UiSavesScreen).mode == &"load", "right turns the app to loading")
	page.select(&"slot_1")
	page.handle(&"confirm")
