...
func test_the_watcher_reads_the_land_under_the_player_after_a_jump() -> void:
	...
	var other := BiomeRegistry.index_of(&"snowfield") if watch.announced != &"snowfield" else BiomeRegistry.index_of(&"moss")
	...
	for i in 3:
		await tree.process_frame
	eq(watch.announced, BiomeRegistry.by_index(other).id, "the watcher read the land under the player with an app up")
	eq(g.hud.place, was, "and nothing was pinged where nobody could read it")
	check(await _tap_until(&"pause", _opened(&"")), "the app closes")
	for i in 3:
		await tree.process_frame
	eq(g.hud.place, BiomeRegistry.by_index(other).display_name, "and the name waiting is pinged then")
	_drop_screens()
	g.free()
...
func test_a_caption_that_waited_is_dropped_when_the_ground_changed_under_it() -> void:
	...
	var other := BiomeRegistry.index_of(&"snowfield") if home_land != &"snowfield" else BiomeRegistry.index_of(&"moss")
	var other_name := BiomeRegistry.by_index(other).display_name
	...
	eq(StringName(ui.get("_pending_place")), BiomeRegistry.by_index(other).id, "the name waits")
