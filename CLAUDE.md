...
| `src/systems/` | `NN_name.gd` game systems, loaded in order: 05 save, 10 sky, 12 landscape, 15 lights, 16 vents, 20 realms, 30 mobs, 35 folk, 36 parade, 37 fauna, 40 fight, 42 target, 44 crafts, 44 sentinels, 50/52 survival, 56 economy, 70 audio, 75 music, 90 ui, 94 dev, 98 tour. |
...
| The plan's depots | `src/core/works/`, `src/systems/34_works.gd` | A region the plan is working keeps a DEPOT (docs/VISION.md §2): `Works.sites(world)` finds one per REGION with a work of the plan already in it and room enough (`MIN_TILES`), at the busiest of that region's marked landmarks, clear of villages and of where the player wakes. Pure and derived, so it is never saved and never moves; what has been DONE to one is (`WorksState`, key `works`). It is where the region's machines come from — one of its own out of the yard every `OWN_EVERY` world minutes and a round along `GenWorks.bearing` every `PATROL_EVERY`, both stopped FOR GOOD when it is broken, which is the whole of "the region quiets" (`Works.bodies_in`). Three working parts (`WorksSite.part(i)`, far enough apart that no two are reachable from one spot) open under a held `use` with a steel edge, each filed as sabotage; the third puts the yard dark, marks every plan work in the yard spent (which is what starves that region's keeper, `Sentinels.feeds`) and starts the land closing over it. `Events.works_broken(region, land)` is the seam for whatever else a lost region means. Reachable by name: `place works`, `works_breaker`, `works_coolant`. Answer a tour with `works`, `works:LAND`, `works_yard`, `works_lit`, `works_dark`, `works_open:N`, `works_part`, `works_broken`, `works_body`, `works_patrol`, `works_pressed`, `works_greening`. |
| Landmarks | `src/core/landmarks/`, `src/systems/22_landmarks.gd` | The places worth the walk (docs/VISION.md §3, §8). A KIND is a `LandmarkDef` in `Landmarks` — the landscapes it stands in, what ground it wants, how far off it can be READ (`sees`, always further than `FOUND_AT`), its drop table and whether something of the plan is still on watch there. A landscape claims its own with `BiomeDef.landmarks`, else every kind that names it; three or more each, held by `Landmarks.problems`. `Landmarks.sites(world)` is pure and derived (one sweep per region, coarse first, remembered per world); only what a player has FOUND and OPENED is saved (key `landmarks`). Its cache stands `CACHE_OUT` in front of its face on ground with nothing solid on it, and a cache that has never been opened takes the `use` key whenever it is in reach. What it gives goes through `src/core/loot` and nothing else — and a landmark may not hold anything a machine carries, because `Sources` walks such a table back to a body and a place is not one. Answer a tour with `landmark`, `landmark:KIND`, `landmark_near`, `landmark_cache`, `landmark_found`, `landmark_opened`, `landmark_guard`, `found:ID`. |
