...
func detail(ci: CanvasItem, r: Rect2i) -> void:
	var box := Rect2i(panel_x(r) + 6, r.position.y + 12, 280, 158)
	if _picture == null:
	...
	match screen.menu.selected().get("id", &""):
		&"restage":
			panel_wrapped(ci, r, y + 2, "A new game at this place, hour, sky and kit, with its saves kept apart. Made in another build, the land may not be the same.")
		&"repro":
			panel_wrapped(ci, r, y + 2, str(note.get("repro", "")))
		_:
			panel_wrapped(ci, r, y + 2, str(note.get("words", "")), UiTheme.TEXT)
