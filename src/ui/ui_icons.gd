...
const SHAPES := {
	...
	&"sticks": [".........", "k.....kk.", "kk...k3k.", ".k2kk32k.", "..k322k..", ".k32k21k.", "k21k.k1k.", "k1k...kk.", "kk......."],
	...
	&"ore_vein": [".........", "..kkkkk..", ".k332wwk.", "k332w221k", "k32ww221k", "kww22111k", ".k21111k.", "..kkkkk..", "........."],
	&"ore_crystal": ["...k.....", "..k3k.k..", "..k2kk3k.", ".kk2kk2k.", ".k12kk2k.", "kk1kkk1kk", "k3322221k", "k2222111k", ".kkkkkkk."],
	&"whelk": ["....k....", "...k3k...", "...k2k...", "..kk3kk..", "..k322k..", ".kk3w2kk.", ".k32221k.", "k3k1111k.", ".kkkkkk.."],
...
const ITEMS := {
	...
	&"limestone": [&"limestone", &"linen", &"linen"],
	&"brimstone": [&"brimstone", &"copper", &"copper"],
	&"timber": [&"timber", &"sand", &"earth"],
	&"driftwood": [&"driftwood", &"ash", &"ash"],
	&"deadwood": [&"sticks", &"earth", &"earth"],
	&"scrap": [&"scrap", &"plate", &"plate"],
	&"mussels": [&"shell", &"slate", &"brine"],
	&"whelks": [&"whelk", &"sand", &"sand"],
	&"samphire": [&"greens", &"moss", &"moss"],
	&"wrack": [&"wrack", &"earth", &"earth"],
	&"reeds": [&"reeds", &"sand", &"sand"],
	&"gorse_cut": [&"gorse", &"spruce", &"copper"],
	&"peat": [&"peat", &"earth", &"earth"],
	&"crottle": [&"crottle", &"ash", &"ash"],
	&"bread": [&"bread", &"earth", &"earth"],
	&"soup": [&"bowl", &"earth", &"moss"],
	&"stew": [&"bowl", &"earth", &"rust"],
	&"smoked": [&"bowl", &"earth", &"copper"],
	&"iron_ore": [&"ore", &"slate", &"rust"],
	&"copper_ore": [&"ore_vein", &"slate", &"copper"],
...
const NEEDS := {
	...
	&"resonance": ["#.......#", "#..#.#..#", "#.#...#.#", "#.#.#.#.#", "#.#...#.#", "#..#.#..#", "#.......#", ".........", "........."],
	&"time_shear": ["#########", ".#.....#.", "..#...#..", "...#.#...", "....#....", "...#.#...", "..#.#.#..", ".#.###.#.", "#########"],
	# Light off a white ground, hard enough to shut an eye: a sun with its rays
	# doubled and the middle burnt out.
	&"glare": ["#..#.#..#", ".#.###.#.", "..#####..", "#########", ".###.###.", "#########", "..#####..", ".#.###.#.", "#..#.#..#"],
	# An empty vessel, cracked: the water you do not have.
	&"thirst": ["..#####..", "..#...#..", "..#.#.#..", "..#.#.#..", "...#.#...", "...#.#...", "...###...", "..#####..", "........."],
	# A field the iron answers: a horseshoe with its arc drawn between the poles.
	&"magnetism": [".##...##.", "###...###", "##.....##", "##.....##", "###...###", ".#######.", "..#####..", "...#.#...", "..#...#.."],
	# Ground giving way: a lintel with its props gone and the fall under it.
	&"collapse": ["#########", "##.....##", ".#.....#.", "..#...#..", "...#.#...", "..#.#.#..", ".#.#.#.#.", "#.#...#.#", "#.......#"],
}
const PRESSURE_ANY := ["...###...", "..#...#..", ".#..#..#.", "#...#...#", "#...#...#", "#.......#", ".#..#..#.", "..#...#..", "...###..."]
