...
##   ... --filter=disposed_harvester --zoom=8 --hour=12   ONE kind standing at each of
...
static func gallery() -> Array:
	...
	if filter.begins_with("disposed"):
		var did := StringName(filter.trim_prefix("disposed_"))
		out.append({"name": filter, "node": disposed(did if KINDS.has(did) else &"harvester", silhouette)})
		return out
...
static func disposed(kid: StringName, silhouette: bool) -> Node3D:
	var holder := Node3D.new()
	var root := Node3D.new()
	var across := Vector3(1, 0, -1).normalized()
	...
	var x := -gap * (Disposition.ORDER.size() - 1) * 0.5
