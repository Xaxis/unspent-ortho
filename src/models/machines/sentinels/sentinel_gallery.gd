...
const KINDS: Array[StringName] = [&"sentinel_reaper", &"sentinel_rake", &"sentinel_plumb", &"sentinel_listener"]
...
static func beside(silhouette: bool) -> Node3D:
	...
	var x := -8.6
	for kid: StringName in [&"harvester", &"sentinel_reaper", &"sentinel_rake", &"sentinel_plumb", &"sentinel_listener"]:
