...
static func make() -> BiomeDef:
	...
	d.adjacency = {&"snowfield": 0.55, &"coast": 0.2}
	...
	d.relief = {
		&"base": 1.6, &"hills": 0.5, &"ridge": 3.2, &"terrace": 0.15, &"valley": 0.3,
	...
	d.decor = {Ground.SNOW: [0.4, Decor.SNOW_TUFT, 18]}
...
static func _surface(t: BiomeSurface, i: int, e: float, rs: float, gb: float, f: int) -> int:
	...
	if rs > 1.2:
		return Ground.ROCK
	return Ground.SNOW if gb > 0.55 else Ground.ICE
