...
## Bodies read at once in a sweep: a field, not a census.
const SWEEP_MOST := 8
...
static func candidates(bodies: Array, from: Vector2, reach: float = REACH) -> Array[MobState]:
	...
	for b: Variant in bodies:
		...
		if m.pos.distance_to(from) > reach:
			continue
		out.append(m)
	out.sort_custom(func(a: MobState, c: MobState) -> bool:
		var ta := threat(a, from)
		var tc := threat(c, from)
		if is_equal_approx(ta, tc):
			# A stable order, so two bodies at one rung never swap as they move.
			return a.id < c.id
		return ta > tc)
	return out
...
static func pick(list: Array[MobState], held: MobState = null) -> MobState:
	if held != null and list.has(held):
		return held
	return list[0] if not list.is_empty() else null


## One step along the list from `held` (dir -1 left, +1 right), wrapping.
static func cycle(list: Array[MobState], held: MobState, dir: int) -> MobState:
	if list.is_empty():
		return null
	var at := list.find(held)
	if at < 0:
		return list[0]
	return list[posmod(at + signi(dir), list.size())]


## What a sweep reads: the front of the list, at most SWEEP_MOST of them.
static func sweep(list: Array[MobState]) -> Array[MobState]:
	return list.slice(0, mini(SWEEP_MOST, list.size()))
...
static func centre_of(list: Array[MobState], player: Vector2) -> Vector2:
	if list.is_empty():
		return player
