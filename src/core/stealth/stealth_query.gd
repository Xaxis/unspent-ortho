...
static func notices(row: Dictionary, from: Vector2, target: Vector2, m: Moment, world: WorldData,
		query: WorldQuery, facing: float = NAN) -> bool:
	# A stolen signet answers their challenge in their own voice: the machine
	# reads one of their own and files nothing, however close you stand. That
	# refusal is the whole of the spoof ability, so it lives at the one door
	# rather than in the gear that grants it. A dog is not fooled by a signet,
	# and neither is anything else alive: only a row that is `machine`.
